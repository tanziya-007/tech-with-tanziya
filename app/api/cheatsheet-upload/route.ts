...
const uploadsDir = join(process.cwd(), 'public', 'uploads', 'cheatsheets');
  ...
  try {
    ...
    if (title && !metadata[slug]) {
      metadata[slug] = {
        ...
        slug,
        category: 'Custom',
        description: `Custom cheat sheet for ${title}`,
        highlights: [],
        topics: [],
        interviews: [],
      };
    ...
    return NextResponse.json({
      ...
      slug,
      filename,
      url: `/uploads/cheatsheets/${filename}`,
      message: 'Cheat sheet image uploaded successfully',
    });
  } catch (error) {
